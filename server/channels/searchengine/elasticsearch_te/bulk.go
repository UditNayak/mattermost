...
// Bulk handles batching of Elasticsearch operations
type Bulk struct {
	mut            sync.Mutex
	logger         mlog.LoggerIFace
	client         *elastic.TypedClient
	bulkClient     *bulk.Bulk
	settings       model.ElasticsearchTESettings
	quitFlusher    chan struct{}
	quitFlusherWg  sync.WaitGroup
	pendingRequests int
}

// NewBulk creates a new bulk processor
func NewBulk(settings model.ElasticsearchTESettings,
	logger mlog.LoggerIFace,
...
// Stop stops the bulk processor and flushes any pending requests
...
// periodicFlusher periodically flushes the bulk processor
